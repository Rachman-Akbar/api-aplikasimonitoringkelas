...
{
    /**
     * Run the database seeds for today's date and future dates.
     */
    public function run(): void
    {
        $today = Carbon::today();
        $this->command->info("Adding records for today and future dates: {$today->toDateString()} and beyond");

        // Add 10 Izin Guru records for today and future dates
        $this->addIzinGuru($today);

        // Add 10 Guru Pengganti records for today and future dates
        $this->addGuruPengganti($today);

        // Add 10 Kehadiran Guru records for today (as they happened today)
        $this->addKehadiranGuru($today);

        // Add 10 Kehadiran Siswa records for today (as they happened today)
        $this->addKehadiranSiswa($today);

        $this->command->info("✅ Today's and future data seeding completed successfully with 10 records each!");
    }
    ...
    private function addIzinGuru($today)
    {
        $gurus = Guru::all();
        if ($gurus->count() > 0) {
            // Create 10 izin records for today and future dates
            for ($i = 0; $i < 10; $i++) {
    ...
    private function addGuruPengganti($today)
    {
        ...
        if ($jadwals->count() > 0 && $gurus->count() > 1) {
            // Create 10 guru pengganti records for today and future dates
            for ($i = 0; $i < 10; $i++) {
    ...
    private function addKehadiranGuru($today)
    {
        ...
        if ($jadwals->count() > 0 && $gurus->count() > 0) {
            // Create 10 kehadiran guru records for today
            for ($i = 0; $i < 10; $i++) {
                // Rotate through schedules and teachers to create different records
                $jadwal = $jadwals[$i % $jadwals->count()];
                $guru = $gurus[$i % $gurus->count()];
    ...
    private function addKehadiranSiswa($today)
    {
        ...
        if ($jadwals->count() > 0 && $siswas->count() > 0) {
            // Create 10 kehadiran siswa records for today
            for ($i = 0; $i < 10; $i++) {
